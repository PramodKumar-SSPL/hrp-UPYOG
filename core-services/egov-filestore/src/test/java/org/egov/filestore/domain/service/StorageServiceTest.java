...
class StorageServiceTest {
	...
	void testSave() {
		...
		when(artifactRepository.save(any(), any())).thenReturn(stringList);
	...
	void testRetrieve() throws IOException {
		...
		StorageValidator storageValidator = new StorageValidator(new FileStoreConfig());
		FileStoreConfig configs = new FileStoreConfig();
		assertNull((new StorageService(artifactRepository, idGeneratorService, fileStoreConfig, storageValidator,
	...
	void testRetrieveNewResource() throws IOException {
		...
		artifact.setTenantId("42");
		ArtifactRepository artifactRepository = mock(ArtifactRepository.class);
		Resource resource = new Resource("text/plain", "foo.txt", new ByteArrayResource("AAAAAAAA".getBytes("UTF-8")),
				"42", "File Size");
		...
		StorageValidator storageValidator = new StorageValidator(new FileStoreConfig());
		FileStoreConfig configs = new FileStoreConfig();
		assertSame(resource, (new StorageService(artifactRepository, idGeneratorService, fileStoreConfig,
				storageValidator, configs, new MinioConfig())).retrieve("foo", "foo"));
		verify(artifact).setContentType((String) any());
		verify(artifact).setCreatedBy((String) any());
		verify(artifact).setCreatedTime((Long) any());
		verify(artifact).setFileName((String) any());
	...
	void testRetrieveByTag() {
		...
		StorageValidator storageValidator = new StorageValidator(new FileStoreConfig());
		FileStoreConfig configs = new FileStoreConfig();
		assertTrue((new StorageService(artifactRepository, idGeneratorService, fileStoreConfig, storageValidator,
				configs, new MinioConfig())).retrieveByTag("foo", "foo").isEmpty());
