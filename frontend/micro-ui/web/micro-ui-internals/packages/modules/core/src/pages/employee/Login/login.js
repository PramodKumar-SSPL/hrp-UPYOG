import { BackButton, Dropdown, FormComposer, Loader, Toast, TextInput, RefreshIcon, Close, ViewsIcon } from "@upyog/digit-ui-react-components";
...
const Login = ({ config: propsConfig, t, isDisabled }) => {
  ...
  const [rawCaptchaId, setRawCaptchaId] = useState("");
...
const setSessionCSRF = (token) => {
  ...
  const fetchCaptcha = async () => {
    try {
      ...
      if (isMountedRef.current) {
        ...
        setRawCaptchaId(captchaIdFromHeader);
        setCaptchaValue("");
      }
    } catch (error) {
  ...
  const onLogin = async (data) => {
    ...
    const encryptedCaptcha = encryptAES(captchaValue);
    const encryptedCaptchaId = encryptAES(rawCaptchaId);
      ...
      <div style={{ width: "100%", position: "fixed", bottom: 0, backgroundColor: "white", textAlign: "center" }}>
        <div style={{ display: "flex", justifyContent: "center", color: "black" }}>
          ...
          <span
            ...
            style={{ cursor: "pointer", fontSize: window.Digit.Utils.browser.isMobile() ? "12px" : "12px", fontWeight: "400" }}
            onClick={() => {
              window.open("https://ulbharyana.gov.in/", "_blank").focus();
            }}
          >
